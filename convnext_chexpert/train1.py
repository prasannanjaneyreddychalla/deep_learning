...
CONFIG: Dict[str, Any] = {
    ...
    # Weights & Biases (forced offline below)
...
# W&B optional, but we will force offline even if available
try:
    import wandb
...
def resolve_path(base_dir: Path, path_in_csv: str) -> Path:
    ...
    for i in range(len(p.parts)):
        ...
        if maybe.exists():
            return maybe
    return cand  # may not exist; callers handle
...
def apply_uncertainty_policy(labels: np.ndarray, policy: str) -> np.ndarray:
    ...
    if policy == "u-zero":
        ...
        raise ValueError("uncertainty_policy must be u-zero, u-one, or ignore")
    return arr
...
def save_curves_npz(npz_path: Path, curves: Dict[str, Dict[str, np.ndarray]]):
    # Convert to plain arrays for np.savez
    arrays = {}
...
def train_one_epoch(model, loader, optimizer, scaler, device, pos_weight, ignore_policy):
    ...
    total, count = 0.0, 0
    for x, y, ignore_mask, _, _ in loader:
...
def evaluate(model, loader, device, pos_weight, ignore_policy, compute_curves: bool = False):
    ...
    for x, y_loss, ignore_mask, y_raw, rel in loader:
        ...
        if ignore_policy == "ignore":
            loss_all = loss_all * (1.0 - ignore_mask)
        loss_sum += loss_all.mean().item() * x.size(0)
        nsum += x.size(0)
...
def main():
    ...
    # Force W&B offline local logging by default
    os.environ["WANDB_MODE"] = "offline"
    os.environ.setdefault("WANDB_PROJECT", CONFIG["WANDB_PROJECT"])
    # optional local dir: keep default to ./wandb
    ...
    if WANDB_LIB_AVAILABLE:
        try:
            wandb.init(
                ...
                name=run_name,
                config=CONFIG,
                mode="offline",  # hard-force offline
            )
    ...
    model = build_convnext_base(len(CHEXPERT_14), pretrained=CONFIG["USE_PRETRAINED"]).to(device)
    ...
    for epoch in range(1, CONFIG["EPOCHS"]+1):
        ...
        # Compute curves on validation each epoch; dataset is small so it's fine
        ...
        cls_pairs = ", ".join(f"{k}:{(val['per_class_auc'][k] if val['per_class_auc'][k]==val['per_class_auc'][k] else float('nan')):.3f}" for k in ["Edema","Pleural Effusion","Atelectasis","Cardiomegaly"])
        ...
        if not improved:
            # patience
            if epoch >= 1 and (epoch - 1) - np.argmin([best_val]) >= CONFIG["PATIENCE"]:
